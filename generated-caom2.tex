...
\section{Model: caom2 }
  ...
  \label{sect:Entity}
    ...
    \subsubsection{Entity.metaChecksum}
      ...
      \textbf{type: \hyperref[sect:ivoa]{ivoa:anyURI}} \newline
      \textbf{multiplicity: 0..1} \newline
      This checksum of the metadata in this entity signals a change in the metadata of an instance and supports validation (e.g. to compare metadata before and after serialisation or persistence). A change in the metaChecksum can also be used to optimise operations like updating a row in the database only when the metaChecksum changed. A change in metaChecksum normally triggers a change in the lastModified timestamp. \par The URI must conform to the pattern {algorithm}:{hex value} (lower case), for example: \begin{verbatim}md5:4be91751541fd804e7207663a0822f56.\end{verbatim} \par The checksum of an entity is computed by accumulating byte representations of individual metadata values in the following order: (1) Entity.id for entities, (2) Entity.metaProducer, (3) state fields in alphabetic order (foo.a comes before foo.b) and using depth-first recursion (foo.abc.x comes before foo.def). The lastModified timestamp is not included in the metaChecksum calculation. Null values are ignored so that the addition of new fields in future versions will not change/invalidate existing checksums. Non-null values are converted to bytes as follows: \begin{itemize} \item string: UTF-8 encoded bytes \item URI: UTF-8 encoded bytes of the string representation \item VocabularyTerm: UTF-8 encoded bytes of the term (do not include the namespace) \item enumeration: the literal value converted to bytes \item float: IEEE754 single (4 bytes) \item double: IEEE754 double (8 bytes) \item boolean: convert to single byte, false=0, true=1 (1 byte) \item byte: as-is (1 byte) \item short: 2 bytes (network byte order == big endian)) \item integer: 4 bytes (network byte order == big endian) \item long: 8 bytes (network byte order == big endian) \item UUID: 16 bytes \item date: treat as a long (milliseconds since 1970-01-01 00:00:00 UTC) \item: array of numbers: treat as a sequence of numbers \item interval: treat as an array of [lower, upper] \item: point: treat as an array of [longitude, latitude] \item: circle: treat as an array of [longitude, latitude, radius] \item: polygon: treat as an array \item list or set of items: treat each item as a simpler type with a single zero (0) byte after each item \item: multiinterval: treat as a list of interval \item: multishape: treat as a list of circle(s) and polygon(s) \end{itemize} When digesting a non-null field value, the field identifier is also converted to bytes and digested (technically, it is the lower case vodml-id value, converted to UTF-8 bytes as above), after the value, to act as a separator between non-null values. Analysis: In the past we have found several obscure ways in which the above set of rules allow for two instances of a model with minor differences to result in the same metadata checkum. The simplest scenario where this happens is when two "adjacent" values in a class are the same type and both can be null; if instance A has the first field set and the other null and instance B has the first field null and the second set to the same value the sequence of bytes is the same and results in the same checksum. This weakness can be fixed by digesting the name of a non-null field after the value. We have no known occurance of this issue in practical use of CAOM - likely because the model is sufficiently complex, but we have observed it when applying the same metaChecksum algorithm in much simpler models that were susceptible to such changes. \par When digesting a list of values, a single zero (0) byte value is digested after each item in the list to act as a separator between values. In principle, two lists (e.g. lists of string keywords) could produce the same byte sequence without this separator: ``abc``, ``def`` and ``abcd``, ``ef``. \par TODO: In the past we have found that some database systems cannot round-trip date values with millisecond precision 100% reliably. The work around was to truncate date values to seconds before converting the long to bytes. TBD
